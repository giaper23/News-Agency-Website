<?php

// This file registers the user in the DB after verifying his email

// Starts session
session_start(); 

// ------ DotEnv Import ----------------------------
require '../vendor/autoload.php';

// Note the different path when not in root folder
$dotenv = \Dotenv\Dotenv::createImmutable(__DIR__ . '/../'); 
$dotenv->load();
//--------------------------------------------------

// Connection to db
require_once 'db_connect.php'; 

// Session variables
$first_name = $_SESSION['first-name'];
$last_name = $_SESSION['last-name'];
$email = $_SESSION['register-email'];
$password = $_SESSION['password'];

// The selector from url
$selector = $_GET['selector'];

// Validator from url in hex
$validator = $_GET['validator']; 

// Empty the success or failed response
$error = ""; 

// If either is empty
if (empty($selector) || empty($validator)) {

    // Error message
    $error = "<div class='alert alert-danger alert-dismissible '><strong>Something went wrong, please try registering again!<strong><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
        
}   else {

    if (ctype_xdigit($selector) !== FALSE && ctype_xdigit($validator) !== FALSE) {

            // Check the date and if db has this selector and validator for this email

            // The current date and time in ms
            $date = date("U");

            // From hex to bin
            $validator = hex2bin($validator);

            // Encrypt validator
            $hashed_validator = md5($validator);

            // Check token in md5 with db
            $check_token_query = "SELECT * FROM email_verification WHERE (selector = '$selector' AND token = '$hashed_validator' AND expires >= '$date')"; 

            // The results from db
            $results = mysqli_query($db, $check_token_query); 

            // If rows return
            if (mysqli_num_rows($results) > 0) {
                
                // Deletes the entry when used
                $delete_used_token_query = "DELETE FROM email_verification WHERE email = '$email'"; 
                
                // The query to delete when not needed anymore
                mysqli_query($db, $delete_used_token_query); 
                
                // Prepare and bind
                $stmt = $db->prepare("INSERT INTO users (first_name, last_name, email, password, image) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("sssss", $first_name, $last_name, $email, $hashed_password, $image);

                $hashed_password = password_hash($password, PASSWORD_DEFAULT);

                // Default profile image
                $image = "data:image/tmp;base64,";

                // Execute statement 
                $stmt->execute();

                // CLose statement
                $stmt->close();

                // Pass the registered user email to another session variable
                $_SESSION['email'] = $_SESSION['register-email'];

                // Unset session variables when not needed anymore
                unset($_SESSION['first-name']);
                unset($_SESSION['last-name']);
                unset($_SESSION['register-email']);
                unset($_SESSION['password']);

                // Close DB connection
                mysqli_close($db);

                // Success message
                echo "<div class='alert alert-success alert-dismissible '><strong>Registration successful! You will be redirected back to our home page in a moment...</strong><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
                
                // Redirects to login page after 5 secs
                header('Refresh: 3; URL=../index.php');

            } else {

                // Error message
                $submit = "<div class='alert alert-danger'><strong>Something went wrong! Please try registering again!</strong></div>"; // Not right selector/validator or 15mins passed and token expired!

            }

        } else {
            // Error message
            $submit = "<div class='alert alert-danger'><strong>Something went wrong! Please try registering again!</strong></div>"; // Not right selector/validator or 15mins passed and token expired!

        }
      
    }
?>